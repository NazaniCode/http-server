...
async function resetContent(req: Request, res: Response, next: NextFunction) {
  res.set("Content-Type", "text/plain; charset=utf-8");
...
async function createChirp(req: Request, res: Response, next: NextFunction) {
  res.set("Content-Type", "application/json");
  type requestData = {
    "body": string,
    "userId": string
  ...
  try {
    ...
    if (chirpBody.length > 140) {
      ...
      const newChirp: NewChirp = await chirpQueries.createChirp({ body: chirpBody, userId: userId });
      const response = {
        "id": newChirp.id,
